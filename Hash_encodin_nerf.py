...
def init_model(is_fine):
    if is_fine:
        ...
        cfg_density_mlp = json.loads(config_density_mlp1)
        cfg_color_mlp = json.loads(config_color_mlp1)
        density_mlp = tcnn.NetworkWithInputEncoding(
            3,
            16,
            encoding_config=cfg_density_mlp["encoding"],
...
class NERF(nn.Module):
    def __init__(self, is_fine):
        super().__init__()
        self.ne1, self.n2, self.e2 = init_model(is_fine)
...
def train(dataset, coarse_model, fine_model, optimizer):
    ...
    for _ in trange(n_iter):
        ...
        if _ % dis_rate == 0:
            ...
            cv2_imshow(rgb_fine.detach().cpu().numpy() * 255)
